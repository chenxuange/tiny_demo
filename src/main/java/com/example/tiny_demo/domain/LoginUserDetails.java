...
public class LoginUserDetails implements UserDetails {
    ...
    public Collection<? extends GrantedAuthority> getAuthorities() {
        List<SimpleGrantedAuthority> authorities = resourceList.stream()
                    .map(role -> new SimpleGrantedAuthority(role.getId() + ":" + role.getName().trim())).collect(Collectors.toList());
            //  若控制器方法上添加@PreAuthorize("hasRole('test')")用作权限控制，那么数据库中必须对应ROLE_test，实际控制器@PreAuthorize("hasRole('ROLE_test')")也可以
            /**
